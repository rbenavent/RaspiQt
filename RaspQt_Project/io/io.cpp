...
io::io(bool restartApp, unsigned int warningTimeut_ms, unsigned int restartTimeut_ms, unsigned int appRestart_ms, QObject* parent) : QThread(parent), m_timerWarning(warningTimeut_ms), m_timerRestart(restartTimeut_ms), m_appRestart_ms(appRestart_ms), m_restart(restartApp) {
    //coherent input data
    if (m_timerWarning < 100)
        m_timerWarning = 100;
    if (m_timerRestart <= m_timerWarning)
        m_timerRestart = m_timerWarning + 1;
    if (m_timerWarning < m_timerMainloopCheck)
        m_timerMainloopCheck = m_timerWarning / 2;

    setObjectName("WDT MainLoop thread");
    //timer mainloop
    m_timer_mainloop.setInterval(m_timerMainloopCheck);
    m_timer_mainloop.setSingleShot(false);
    connect(&m_timer_mainloop, &QTimer::timeout, this, &io::resetWdtMainloop, Qt::DirectConnection);
    //timer thread
...
void io::resetWdtMainloop() {
    QMutexLocker m(&m_mutex);
    m_tcpp.restart();
    m_half = false;
}

void io::forceExit [[noreturn]] () {
...
void io::forceRestart() {
...
#ifdef Q_OS_WINDOWS
        if (m_appRestart_ms < 1000)
            m_appRestart_ms = 1000;
    ...
    for (auto& val : list)
        argsCopy.push_back(val.toStdString());
